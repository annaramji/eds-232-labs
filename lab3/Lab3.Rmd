...
**The x-axes are showing the values of lambda (log-transformed), and the x-axis shows the values of Mean-Squared Error. Each point on the line for the graphs shows the MSE at each value of lambda (red line showing average, grey showing variation over the 10 runs). The numbers at the top of the plots show the number of coefficients in the model (constant for Ridge, decreasing for LASSO as it performs feature selection). In both models, as lanbda increases, the MSE also increases.**
...
# Lambda for min MSE
print(paste("The lambda value associated with the minimum MSE is",
...
lasso_abd$nzero[lasso_abd$lambda == lasso_abd$lambda.1se]

```

-   ***Question 9*****.** Which regularized regression worked better for this task, ridge or lasso? Explain your answer.

Lasso seems to have worked better for this task for a couple of reasons. First, the minimum mean squared error (MSE) is lower for Lasso than Ridge -- at around 4.77 (with a lambda value of 0.002) and 5.1 (with a lambda value of 0.2) respectively. Second, the Lasso model has feature selection, with 5 coefficients selected within 1 standard error (SE) of the MSE. This is relevant to evaluating which regularized regression model worked better because having fewer features, combined with a lower MSE, sets us up for less collinearity or potential correlation between terms, and also reduces the risk of overfitting to too many variables.

Thus, overall, Lasso is a better regularized regression model for this task because it has lower minimum MSE and MSE within 1 SE from the minimum fewer predictors -- having fewer features = less collinearity, and more features can lead us to be at risk for more overfitting.
