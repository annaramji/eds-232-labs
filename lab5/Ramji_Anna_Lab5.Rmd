...
# set initial offset (starting point)
offset = 0

# create function to iteratively retrieve tracks data
track_function <- function(tracks) {
  for (i in 1:tracks){
    # for each iteration, get new tracks
    new_tracks <- get_my_saved_tracks(
      # using this authorization code
      authorization = authorization_code,
      # specifying a limit of 50
      limit = 50,
      # set the offset to whatever the offset is at this iteration
      offset = offset)
    
    # after retrieving that data, add 50 to the offset for the next iteration
    offset = offset + 50
    
    # and append / save tracks to the initial dataframe
    all_my_tracks <- rbind(all_my_tracks, new_tracks)
    
  }
  
  # return the final dataframe with all rows (specified by number of iterations x 50)
  return(all_my_tracks)
  
}

# retrieving information on 4 x 50 (= 200) tracks, saving as my_tracks
my_tracks <- track_function(tracks = 4)
```
...
```{r}
# appending track.name column to my_tracks
named_audio <- my_tracks |>
  select(track.name) |>
  bind_cols(audio_features)
  
# write this to a csv for sharing with Sam (lab partner)
...
```{r}
# adding my name to my dataframe
my_audio_data <- named_audio |> 
  # creating collection column to distinguish whose data is whose

  mutate(collection = "Anna") 
# reading in partner's data, adding their name to their data
sams_data <- read_csv(here("lab5", "data", "sam_audio.csv")) |> 
  mutate(collection = "Sam")

# joining our dataframes, making the collection ID a factor for classification
...
combined_audio_clean <- combined_audio_df |> 
  #  removing columns that don't seem like they're going to be helpful
  # in making our model 
  select(-c(track.name, id, uri, track_href, analysis_url, type)) 
...
# view the top 3 tracks by danceability
top_danceable <- combined_audio_df |> 
  arrange(desc(danceability)) |> 
  select(track.name, danceability, collection) |> 
  head(3)

top_dance_tracks <- top_danceable |> gt::gt() |> 
...
dance_plot <- ggplot(data = combined_audio_clean) +
  geom_density(aes(x = danceability, 
               ...
               alpha = 0.5) +
  theme_bw() +
  # removing legend for patchwork aesthetics
  guides(fill = "none") + 
  labs(x = "Danceability",
       y = "Density",
       # commenting out legend title for patchwork aesthetics
#       fill = "Collection",
       title = "Danceability, Energy, and Acousticness Density plots per Collection")
...
# view the top 3 tracks by energy
top_energy <- combined_audio_df |> 
  arrange(desc(energy)) |> 
  select(track.name, energy, collection) |> 
  head(3)

top_energy_tracks <- top_energy |> gt::gt() |> 
  gt::opt_stylize(style = 2,
                  color = "gray")
...
energy_plot <- ggplot(data = combined_audio_clean) +
  ...
  labs(x = "Energy",
       y = "Density",
       fill = "Collection",
       # commenting out title for patchwork assembly aesthetics
   #    title = "Energy Density plot per Collection"
       )

# Acousticness --------------

# find most acoustic track
...
# view the top 3 tracks by acousticness
top_acoustic <- combined_audio_df |> 
  arrange(desc(acousticness)) |> 
  select(track.name, acousticness, collection) |> 
  head(3)

top_acoustic_tracks <- top_acoustic |> gt::gt() |> 
  gt::opt_stylize(style = 2,
                  color = "gray")
...
acc_plot <- ggplot(data = combined_audio_clean) +
  ...
  labs(x = "Acousticness",
       y = "Density",
       # commenting out title and legend label for patchwork aesthetics
  #     fill = "Collection",
...
# viewing top 3 tracks by danceability, energy, and acousticness
...
Our music appears to be relatively equally danceable, and while I have listened to the song with the highest energy (Panama - 2015 Remaster (a Van Halen classic!)), looking at the density plots we can see that Sam's music taste seems to have more high-energy songs, whereas my taste is roughly normally distributed. I also appear to like more acoustic music than Sam, though we are tied for the most acoustic track. 
...
# ------------- Fitting, assessing model performance ----------------
...
# ------------ Fitting final workflow, assessing model performance ------------
final_fit <- final_wf |>
...
knn_conf_mat_roc <- test_predict_roc |> 
  conf_mat(truth = collection, estimate = .pred_class) |>  #create confusion matrix
  autoplot(type = "heatmap") + #plot confusion matrix with heatmap
  theme_bw() + #change theme
  theme(axis.text.x = element_text(angle = 30, hjust=1)) + #rotate axis labels
...
# ----------- Fit model, assessing model performance ---------------------
...
```{r bagged-tree}
set.seed(123) # pseudo-random for reproducibility
...
# viewing best options based on accuracy and roc_auc
...
# --------- Fitting, assessing model performance ------------------
#fit the RF model to the training set for roc_auc and accuracy-optimized models
train_fit_rf <- fit(rf_final_roc_auc, audio_train) 
...
# (commented out because I'll compare them all at the end)
...
performance_metrics <- rbind(knn_metrics,
                             tree_metrics,
                             bag_metrics,
                             rf_metrics
                             ) |> 
...
This table shows us that the best performing models for our data are the Random Forest and Bagged Tree model (roc_auc slightly higher for Random Forest than Bagged Trees), with roc_auc and accuracy scoring slightly higher than all other models, other than KNN, which has a higher roc_auc score. Based on the confusion matrices, the Random Forest model seems to perform the best on our data, with the fewest false negatives and false postives compared to true positives and true negatives. 
...
comparison_bar_plot <- ggplot(data = performance_metrics) +
  ...
  scale_fill_manual(values = c("plum", "darkmagenta")) +
  theme_bw() +
  labs(x = "Model",
       y = "Estimate",
       fill = "Metric",
