...
We're working with elevation, slope, aspect, physical characteristics of the topography and area, including: horizontal distance to roadways, hydrology, and fire points; hillshade at 9, noon, and 3; wilderness area; and 41 soil types (Soil_Type, Soil_Type_1, .... Soil_Type_40). Currently these values are all doubles. 
...
**I ended up not using the same recipe for both models 
...
cv_folds <- vfold_cv(train, v = 5, strata = cover_type)
```
...
#     svm_wf,
#     resamples = cv_folds,
#     grid = svm_grid # 125 parameter combinations
...
# finalize workflow  ----
best_cost <- select_best(svm_rs, metric = "accuracy")
...
random forest - resplit training data 5% (0.05) of training data, 2 fold, grid = 3
...
random forest good at working through class imbalance. SVM built for binary, random forest better for multiclass classifying
